...
namespace Bookstore.Controllers
{
    ...
    public class ReservationController : ControllerBase
    {
        ...
        [HttpGet("{id}")]
        public async Task<Reservation> GetReservation(string id) => await _mediator.Send(new GetReservationByBookId.Query { Id = id });

        [HttpPost]
        public async Task<ActionResult> CreateReservation([FromBody] AddNewReservation.Command command)
        {
            var createReservationId = await _mediator.Send(command);
            return CreatedAtAction(nameof(GetReservation), new { id = createReservationId }, null);
        }
    }
}
